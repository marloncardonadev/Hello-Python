...
async def user(user: User):
    if type(search_user("email", user.email)) == User:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="El usuario con el email: " + str(user.email) + " ya existe.")
...
async def user(id: str):
    ...
    if not found:
        return {"Error": "No se elimino el usuario con el id: " + id}
    return {"Ok": "Se elimino el usuario con el id: " + id}
